...
def batch_norm_relu(inputs, is_training, data_format):
    inputs = tf.layers.batch_normalization(inputs=inputs, axis=1 if data_format == 'channels_first' else 3, 
        momentum=_BATCH_NORM_DECAY, epsilon=_BATCH_NORM_EPSILON, center=True, scale=True, is_training=is_training, fused=True)
...
def conv2d_fixed_padding(inputs, filters, kernel_size, strides, data_format):
    ...
    return tf.nn.conv2d(inputs=inputs, filters=filters, kernel_size=kernel_size, strides=strides, data_format=data_format,
        padding = ('SAME' if strides==1 else 'VALID'), use_bias=False, kernel_initializer = tf.variance_scaling_initializer())
        
        
def building_block(inputs, filters, is_training, projection_shortcut, strides, data_format):
    # with BN before convolutions
...
def cifar10_resnet_v2_generator(resnet_size, num_classes, data_format=None):
    ...
    def model(inputs, is_training):
        ...
        inputs = block_layer(inputs=inputs, filters=16, block_fn=building_block, 
            blocks=num_blocks, strides=1, is_training=is_training, name='block_layer1', data_format=data_format)
            
        inputs = block_layer(inputs=inputs, filters=32, block_fn=building_block, 
            blocks=num_blocks, strides=2, is_training=is_training, name='block_layer2', data_format=data_format)        
        
        inputs = block_layer(inputs=inputs, filters=64, block_fn=building_block, 
            blocks=num_blocks, strides=2, is_training=is_training, name='block_layer3', data_format=data_format)    
    
    
        inputs = batch_norm_relu(inputs, is_training, data_format)
        inputs = tf.layers.average_pooling2d(inputs=inputs,pool_size=8,strides=1,padding='VALID',data_format=data_format)
        
        inputs = tf.idendity(inputs,'final_avg_pool')
...
def imagenet_resnet_v2_generator(block_fn, layers, num_classes, data_format=None):    
    ...
    def model(inputs, is_training):
        ...
        inputs = tf.identity(inputs, name='initial_conv')
...
def imagenet_resnet_v2(resnet_size, num_classes, data_format=None):
    ...
    if resnet_size not in model_params:
        raise ValueError('resnet size should be 18, 34, 50, 101, 152, or 200')
        
    params = model_params[resnet_size]
    return imagenet_resnet_v2_generator(params['block'], params['layers'], num_classes, data_format)
